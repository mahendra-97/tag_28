    ...
    def post(self, request):
        ...
        try:
            ...
            if action == 'assign':
                ...
                # Split the comma-separated string into a list of VM IDs
                # vm_ids = [vm_id.strip() for vm_id in vm_ids_str.split(',') if vm_id]
    ...
    def post(self, request):
        try:
            ...
            if tag_name:
                ...
                tag_instance = TagsModel.objects.filter(tag_name=tag_name).first()

                if tag_instance is None:
                    tag_instance = TagsModel.objects.create(tag_name=tag_name, scope=scope)

                # result = TagsModel.objects.get_or_create(tag_name=tag_name, scope=scope, user_id=user_profile)
